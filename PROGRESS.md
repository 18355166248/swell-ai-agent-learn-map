# 学习进度追踪

> 每周五更新一次，记录本周完成情况和下周计划。

## 总体进度

| 周次   | 时间        | 主题                    | 状态      | 项目产出           |
| ------ | ----------- | ----------------------- | --------- | ------------------ |
| Week 1 | 05/21–05/27 | Prompt 基础 + LLM API   | 🟡 进行中 | ai-code-explain v1 |
| Week 2 | 05/28–06/03 | LLM API 进阶 + 文件问答 | ⬜ 未开始 | ai-code-explain v2 |
| Week 3 | 06/04–06/10 | Embedding + 向量检索    | ⬜ 未开始 | doc-rag 骨架       |
| Week 4 | 06/11–06/17 | RAG 完整 Demo           | ⬜ 未开始 | doc-rag 完整版     |
| Week 5 | 06/18–06/24 | RAG 效果优化            | ⬜ 未开始 | 实验记录           |
| Week 6 | 06/25–07/01 | 接入真实场景            | ⬜ 未开始 | req-analyst v1     |
| Week 7 | 07/02–07/08 | Agent + 工具调用        | ⬜ 未开始 | dev-copilot v1     |
| Week 8 | 07/09–07/15 | 整合收尾                | ⬜ 未开始 | dev-copilot 完整版 |

状态说明：✅ 完成 | 🟡 进行中 | ⬜ 未开始 | ❌ 延期

---

## Week 1（05/21–05/27）

### 完成情况

- [ ] 理解 System Prompt / User Prompt
- [ ] 掌握 JSON 输出约束
- [ ] 了解 Function Calling 概念
- [ ] 知道 Temperature/max_tokens 的作用
- [ ] 完成 ai-code-explain CLI v1

### 本周笔记

> 见 `weeks/week-01/notes.md`

### 踩坑记录

> 在这里补充...

### 下周计划预览

- streaming 输出
- 长文档处理
- 支持 --file 参数

---

## Week 2（05/28–06/03）

### 完成情况

- [ ] streaming 流式输出
- [ ] token 计算和上下文窗口
- [ ] 长文档处理（截断/摘要）
- [ ] 错误处理和重试
- [ ] 完成 ai-code-explain v2（支持 --file 和 --dir）

### 本周笔记

> 见 `weeks/week-02/notes.md`

### 踩坑记录

> 在这里补充...

---

## Week 3（06/04–06/10）

### 完成情况

- [ ] 用 Embedding API 向量化文本
- [ ] 手写 Cosine Similarity
- [ ] 实现文档切分
- [ ] 批量 embedding 并存到 JSON
- [ ] 实现最基础的向量检索

### 本周笔记

> 见 `weeks/week-03/notes.md`

---

## Week 4（06/11–06/17）

### 完成情况

- [ ] 完整 RAG Pipeline
- [ ] 引用来源展示
- [ ] 基础 Web UI
- [ ] 端到端测试通过

### 本周笔记

> 见 `weeks/week-04/notes.md`

---

## Week 5（06/18–06/24）

### 完成情况

- [ ] 建立评估数据集（10 个问题）
- [ ] 对比 3 种 Chunk 策略
- [ ] 实现 Query 改写
- [ ] 调研 BM25 关键词搜索
- [ ] 最优策略回填进项目

### 实验结论

> 见 `experiments/chunk-strategies/`

---

## Week 6（06/25–07/01）

### 完成情况

- [ ] 收集并整理真实文档
- [ ] 建立文档索引
- [ ] 完成需求分析 Prompt 设计
- [ ] 结构化输出实现
- [ ] 真实需求测试 ≥ 3 个

---

## Week 7（07/02–07/08）

### 完成情况

- [ ] Tool Calling 实现
- [ ] read_file 工具
- [ ] search_code 工具
- [ ] search_docs 工具
- [ ] 多工具组合 Agent 测试

---

## Week 8（07/09–07/15）

### 完成情况

- [ ] 功能整合完毕
- [ ] README 和使用文档
- [ ] 学习总结文档
- [ ] 技术分享材料（可选）

---

## 关键指标

| 指标           | 目标          | 当前 |
| -------------- | ------------- | ---- |
| 周产出 Demo 数 | 每周至少 1 个 | 0    |
| 笔记篇数       | 每周 1 篇     | 0    |
| 实验记录       | ≥ 3 组对比    | 0    |
| 最终项目能力点 | 6 项          | 0    |
