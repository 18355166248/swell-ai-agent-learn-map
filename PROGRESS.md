...
- 更新时间：2026-05-31
- 当前阶段：Week 9 进行中
- 当前重点：Phase 2A · 建立 RAG / Agent 评估体系，从"做功能"进入"证明变好"

### 最新里程碑

- `05-agent-eval` 已支持按轮次重跑（`--round`）和按环境变量锁定模型（`MODEL_NAME`）
- 评估日志会打印当前轮次和模型，便于确认是否真的命中目标配置
- `04-dev-copilot` 经过多轮 prompt 收敛后，`agent-eval-round-01` 在 `round-4` 达到 `5/5 = 100%`
- 当前下一步是用同一模型继续跑 `eval:all` 总盘，确认 RAG / Agent / Req-Analyst 三条链路是否同时稳定
...
| Week 9 | —           | Phase 2A · Evaluation   | 🟡 进行中 | agent-eval 评估体系 |
